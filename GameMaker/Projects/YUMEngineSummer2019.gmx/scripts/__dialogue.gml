...
#define __dialogue_check_hold_timer
/// __dialogue_check_hold_timer(raw_input, timer)
    var raw_input = argument0, timer = argument1;
    var sys = __get_dialogue_system();
    if raw_input {
        if timer == 0 {
            return true;
        }
        if timer >= sys._input_hold_delay {
            var delta = timer - sys._input_hold_delay;
            if delta % sys._input_hold_interval == 0 {
                return true;
            }
        }
        return false;
...
/// __dialogue_check_up()
    var sys = __get_dialogue_system();
    return __dialogue_check_hold_timer(sys._input_up_raw, sys._input_up_timer);
#define __dialogue_check_down
/// __dialogue_check_down()
    var sys = __get_dialogue_system();
    return __dialogue_check_hold_timer(sys._input_down_raw, sys._input_down_timer);
#define __dialogue_check_left
/// __dialogue_check_left()
    var sys = __get_dialogue_system();
    return __dialogue_check_hold_timer(sys._input_left_raw, sys._input_left_timer);
#define __dialogue_check_right
/// __dialogue_check_right()
    var sys = __get_dialogue_system();
    return __dialogue_check_hold_timer(sys._input_right_raw, sys._input_right_timer);
...
/// dialogue_up(up_input)
    ...
    var sys = __get_dialogue_system();
    sys._input_up_raw = up_input;
#define dialogue_down
/// dialogue_down(down_input)
    var down_input = argument0;
    var sys = __get_dialogue_system();
    sys._input_down_raw = down_input;
#define dialogue_left
/// dialogue_left(left_input)
    var left_input = argument0;
    var sys = __get_dialogue_system();
    sys._input_left_raw = left_input;
#define dialogue_right
/// dialogue_right(right_input)
    var right_input = argument0;
    var sys = __get_dialogue_system();
    sys._input_right_raw = right_input;
...
/// __dialogue_draw_text_ext(xpos, ypos, text, stringlength, sep, w)
    ...
    for (var i = 1; i <= stringlength; ++i) {
        ...
        if draw {
            ...
            var col_h = color_get_hue(col),
                col_s = color_get_saturation(col),
                col_v = color_get_value(col);
            var col_darker = make_color_hsv(col_h, col_s, col_v / 2);
            var col_shadow = make_color_hsv(col_h, 0, 50);
            var tx = xpos + current_w,
                ty = ypos + current_h;
            // Draw the shadow and then the faded top text
            draw_text_color(tx + 1, ty + 1, char, col_shadow, col_shadow, col_shadow, col_shadow, draw_get_alpha());
            draw_text_color(tx, ty, char, col, col, col_darker, col_darker, draw_get_alpha());
            current_w += string_width(char);
            // Handle w and sep
            if current_w > w && w != -1 {
                current_w = 0;
...
#define dialogue_draw_cursor_timed
/// dialogue_draw_cursor(xpos, ypos, width, height, time)
    var xpos = argument0, ypos = argument1, width = argument2, height = argument3, time = argument4;
...
/// dialogue_draw_choice(xpos, ypos, width, height, choices, selected, time)
    ...
    for(var i = 0; i < array_length_1d(choices); ++i) {
        ...
        if i == selected {
            ...
            dialogue_draw_cursor_timed(
                tx - bump, ty - bump,
               width - 2*tpx + 2*bump, text_height + 2*bump, time
            );
            //draw_set_alpha(1);
        }
...
#define dialogue_opener_set_open_auto
/// dialogue_opener_set_open_auto(opener, is_open)
    var opener = argument0, is_open = argument1;
    // Opens or closes a dialogue, waiting for it to be open or closed before
    // doing so.
    if is_open {
        if dialogue_opener_is_closed(opener)
            dialogue_opener_open(opener);
    } else {
        if dialogue_opener_is_open(opener)
            dialogue_opener_close(opener);
    }
...
#define dialogue_get_selection_bump
/// dialogue_get_selection_bump()
    return 4 * __dialogue_get_scale_factor() * 2;