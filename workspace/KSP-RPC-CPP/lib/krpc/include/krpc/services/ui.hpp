...
class UI : public Service {
 public:
  ...
  ::krpc::schema::ProcedureCall message_call(std::string content, float duration, UI::MessagePosition position);
...
inline ::krpc::schema::ProcedureCall UI::message_call(std::string content, float duration = 1.0, UI::MessagePosition position = static_cast<UI::MessagePosition>(1)) {
  std::vector<std::string> _args;
  _args.push_back(encoder::encode(content));
  _args.push_back(encoder::encode(duration));
  _args.push_back(encoder::encode(position));
