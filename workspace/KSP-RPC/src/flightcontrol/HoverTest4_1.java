...
public class HoverTest4_1 extends RPCClient {

	private static final String VESSEL_NAME = "HoverPad 2";
	...
	private List<HoverTest4Vessel> myVessels;

	private double theta;

	public HoverTest4_1(String serverName, String serverIPAddr, int serverPortRPC,
			int serverPortStream) {
		super(serverName, serverIPAddr, serverPortRPC, serverPortStream);

		myVessels = new ArrayList<HoverTest4Vessel>();
	}
	...
	protected void setup() throws RPCException {
		...
		for (Vessel vessel : spaceCenter.getVessels()) {
			if (vessel.getName().equals(VESSEL_NAME)) {
				myVessels.add(new HoverTest4Vessel(vessel, targetVessel, 110, 0));
			}
		}

//		myVessels.get(0).setDeltaTarget(0.001, 0.001);
//		myVessels.get(1).setDeltaTarget(-0.001, 0.001);
//		myVessels.get(2).setDeltaTarget(-0.001, -0.001);
//		myVessels.get(3).setDeltaTarget(0.001, -0.001);
		myVessels.get(0).setThetaOffset(0);
		myVessels.get(1).setThetaOffset(Math.PI / 2.0);
		myVessels.get(2).setThetaOffset(Math.PI);
		myVessels.get(3).setThetaOffset(Math.PI * 1.5);

		for(HoverTest4Vessel vessel : myVessels) {
			vessel.setup();
		}
	}
	...
	protected void update() throws RPCException {
		theta = Math.PI * (System.currentTimeMillis() / 1000.0) / 20f;
		System.out.println(Math.toDegrees(theta) % 360);
		for(HoverTest4Vessel vessel : myVessels) {
			vessel.update();
		}
	}

	private class HoverTest4Vessel {
...
//		private double targetLatitude, targetLongitude;
		...
		public HoverTest4Vessel(Vessel vessel, Vessel targetVessel, double altitude, double thetaOffset) {//, double deltaLatitude, double deltaLongitude) {
			this.vessel = vessel;
...
//		private void setDeltaTarget(double deltaLatitude, double deltaLongitude) {
//			this.deltaLatitude = deltaLatitude;
//			this.deltaLongitude = deltaLongitude;
//		}
		private void setThetaOffset(double thetaOffset) {
			this.thetaOffset = thetaOffset;
		}

		private boolean closeEnough(double a, double b, double closeEnough) {
...
//		@Override
		protected void update() throws RPCException {
			double targetLatitude = targetFlight.getLatitude() + 0.002 * Math.cos(thetaOffset + theta);
			double targetLongitude = targetFlight.getLongitude() + 0.002 * Math.sin(thetaOffset + theta);
...
//			System.out.println("custom val: " + positionLonPID.tempGetDPart());
	...
	public static void main(String[] args) {
		new HoverTest4_1("RPC Server", "127.0.0.1", 50000, 50001).start();
	}
}
